...
export default function Mail() {
  ...
  // Combine and filter items based on folder
    ...
    switch (selectedFolder) {
      case "unread":
        filtered = items.filter((item) => {
          if (item.type === "email") return !(item.data as Email).processed;
          return true;
        });
        break;
      case "starred":
        // For now, no star functionality yet - show empty
        filtered = [];
        break;
      case "sent":
        filtered = emails
          .filter((e) => e.folder === "SENT")
          .map((email) => ({
            id: `email-${email.id}`,
            type: "email" as const,
            data: email,
  ...
  }, [tasks, emails, selectedFolder, searchQuery]);
  ...
  const counts = {
    inbox: tasks.length + emails.filter((e) => !e.processed).length,
    unread: emails.filter((e) => !e.processed).length + tasks.length,
