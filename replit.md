...
- **Calendar Integration**: Google Calendar connected via Replit integration for viewing upcoming events within the next 7 days from ALL user calendars (not just primary). Events display their source calendar name and color. Supports both timed and all-day events with proper validation. Email integration requires Gmail App Password (not regular password) for authentication.
